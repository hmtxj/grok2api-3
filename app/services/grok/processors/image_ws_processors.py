...
class ImageWSBaseProcessor(BaseProcessor):
    ...
    def _to_output(self, image_id: str, item: Dict) -> str:
        try:
            ...
            # 打码检测：配置启用时跳过模糊图片
            if b64 and get_config("image.filter_blur_cache", False):
                from app.services.grok.utils.blur_detector import is_blurry
                if is_blurry(b64):
                    logger.debug(f"跳过打码图片: {image_id}")
                    return ""
            if self.response_format == "url":
                return self._save_blob(
                    image_id, item.get("blob", ""), item.get("is_final", False)
                )
            # b64_json 模式下，final 图片也保存一份到本地缓存
            if b64 and item.get("is_final", False):
                self._save_blob(image_id, blob, True)
            return b64

...
function getLightboxItems() {
  return cacheListState[lightboxType]?.items || [];
}

function openLightbox(type, index) {
  const dialog = byId('lightbox-dialog');
  if (!dialog) return;
  lightboxType = type;
  lightboxIndex = index;
  showLightboxImage();
...
function showLightboxImage() {
  ...
  if (nextBtn) nextBtn.disabled = lightboxIndex >= items.length - 1;
}
...
async function lightboxDeleteCurrent() {
  const items = getLightboxItems();
  const item = items[lightboxIndex];
  if (!item) return;
  const ok = await confirmAction(`确定要删除 ${item.name} 吗？`, { okText: '删除' });
  if (!ok) return;
  const okDelete = await requestDeleteLocalFile(lightboxType, item.name);
  if (!okDelete) { showToast('删除失败', 'error'); return; }
  showToast('删除成功', 'success');
  const state = cacheListState[lightboxType];
  if (state && Array.isArray(state.items)) {
    state.items = state.items.filter(i => i.name !== item.name);
    state.loaded = true;
    selectedLocal[lightboxType]?.delete(item.name);
    if (state.visible) renderLocalCacheList(lightboxType, state.items);
  }
  loadStats();
  const newItems = getLightboxItems();
  if (newItems.length === 0) { closeLightbox(); return; }
  if (lightboxIndex >= newItems.length) lightboxIndex = newItems.length - 1;
  showLightboxImage();
}

function lightboxDownloadCurrent() {
  const items = getLightboxItems();
  const item = items[lightboxIndex];
  if (!item) return;
  const safeName = encodeURIComponent(item.name);
...
document.addEventListener('DOMContentLoaded', () => {
  const closeBtn = byId('lightbox-close');
  const backdrop = byId('lightbox-backdrop');
  const prevBtn = byId('lightbox-prev');
  const nextBtn = byId('lightbox-next');
  const downloadBtn = byId('lightbox-download');
  const deleteBtn = byId('lightbox-delete');
  ...
  if (downloadBtn) downloadBtn.addEventListener('click', lightboxDownloadCurrent);
  if (deleteBtn) deleteBtn.addEventListener('click', lightboxDeleteCurrent);
});
...
// ==================== 任务分组视图 ====================
let currentCacheView = 'list';
let taskPage = 1;
const taskPageSize = 20;
let taskTotal = 0;
let filterBlurry = false;
const blurCache = new Map();
let _lastTasksData = [];
let _lastUncategorizedData = [];

// 相邻像素差异分析法检测模糊打码图（移植自 Imagine 瀑布流）
function isBlurryByUrl(imageUrl) {
  return new Promise((resolve) => {
    const img = new Image();
    ...
    img.onload = () => {
      try {
        ...
        resolve(avgDiff < 4 || sharpRatio < 3);
      } catch (e) {
        resolve(false);
      }
    };
...
async function applyBlurFilter() {
  const thumbs = document.querySelectorAll('.task-image-thumb, .uncategorized-thumb');
  const promises = Array.from(thumbs).map(async (thumb) => {
    // 用原始相对路径作为 key，与 img.view_url 保持一致
    const key = thumb.getAttribute('src');
    if (!key) return;
    let isBlur = blurCache.get(key);
    if (isBlur === undefined) {
      isBlur = await isBlurryByUrl(thumb.src);
      blurCache.set(key, isBlur);
    }
    thumb.style.display = isBlur ? 'none' : '';
  });
  await Promise.all(promises);
  // 更新各分组的图片计数
  document.querySelectorAll('.task-card').forEach(card => {
    const visibleCount = card.querySelectorAll('.task-image-thumb:not([style*="display: none"])').length;
    const countSpan = card.querySelector('.task-img-count');
    if (countSpan) countSpan.textContent = `${visibleCount} 张`;
    // 如果全部隐藏则隐藏整个卡片
...
function removeBlurFilter() {
  document.querySelectorAll('.task-image-thumb, .uncategorized-thumb').forEach(thumb => {
    thumb.style.display = '';
  });
  document.querySelectorAll('.task-card').forEach(card => {
    card.style.display = '';
    // 恢复原始计数
    const totalCount = card.querySelectorAll('.task-image-thumb').length;
    const countSpan = card.querySelector('.task-img-count');
    if (countSpan) countSpan.textContent = `${totalCount} 张`;
  });
}

function switchCacheView(view) {
  currentCacheView = view;
  const switcher = byId('view-switcher');
  if (switcher) {
    switcher.querySelectorAll('.tab-item').forEach(btn => {
  ...
  const listView = byId('local-cache-lists');
  const taskView = byId('task-group-view');

  if (view === 'tasks') {
    if (listView) listView.classList.add('hidden');
    if (taskView) taskView.classList.remove('hidden');
    loadTaskGroups();
  } else {
...
async function loadTaskGroups() {
  const container = byId('task-group-list');
  const empty = byId('task-group-empty');
  const pagination = byId('task-pagination');
  if (!container) return;

  container.innerHTML = '<div class="text-center py-8 text-gray-400 text-sm">加载中...</div>';

  try {
    const res = await fetch(`/api/v1/admin/cache/tasks?page=${taskPage}&page_size=${taskPageSize}`, {
      headers: buildAuthHeaders(apiKey)
    });
    const data = await res.json();
    ...
    // 更新 tab 计数
    const countEl = byId('task-view-count');
    if (countEl) countEl.textContent = taskTotal;
    ...
    if (tasks.length === 0 && taskPage === 1) {
      ...
      renderTaskGroups(container, tasks);
    }

    // 分页
    const totalPages = Math.ceil(taskTotal / taskPageSize);
    if (totalPages > 1) {
      if (pagination) pagination.classList.remove('hidden');
      const info = byId('task-pagination-info');
      if (info) info.textContent = `第 ${taskPage} / ${totalPages} 页 · 共 ${taskTotal} 个任务`;
      const prev = byId('task-page-prev');
      const next = byId('task-page-next');
      if (prev) prev.disabled = taskPage <= 1;
      if (next) next.disabled = taskPage >= totalPages;
    } else {
    ...
    container.innerHTML = `<div class="text-center py-8 text-red-500 text-sm">加载失败: ${e.message}</div>`;
  }
}
...
function renderTaskGroups(container, tasks) {
  ...
  tasks.forEach((task, idx) => {
    ...
    const promptShort = task.prompt.length > 60
    ...
    const sourceBadge = task.source === 'imagine'
      ? '<span class="badge badge-orange">Imagine</span>'
      : '<span class="badge badge-gray">API</span>';

    card.innerHTML = `
      <div class="task-card-header">
        <div class="task-card-header-left" onclick="toggleTaskCard(this.parentElement)">
          <svg class="task-card-arrow" width="14" height="14" viewBox="0 0 24 24" fill="none"
            stroke="currentColor" stroke-width="2">
            <polyline points="6 9 12 15 18 9"></polyline>
          </svg>
          <div class="task-card-info prompt-tooltip" data-tooltip="${escapeAttr(task.prompt)}">
            <div class="task-card-prompt">${escapeHtml(promptShort)}</div>
            <div class="task-card-meta">
              ${sourceBadge}
              <span class="task-img-count">${images.length} 张</span>
              <span>${task.aspect_ratio || ''}</span>
              <span>${timeStr}</span>
            </div>
          </div>
        </div>
        <button class="task-delete-btn" title="删除分组" onclick="event.stopPropagation(); deleteTaskGroup('${task.task_id}', '${escapeAttr(promptShort)}')">
      ...
      <div class="task-card-body" data-task-id="${task.task_id}">
        <div class="task-image-grid">
          ${images.map((img, i) => `
            <img src="${img.view_url}" alt="" class="task-image-thumb" loading="lazy"
              data-img-name="${escapeAttr(img.name)}"
              onclick="openLightboxFromTask('${task.task_id}', '${escapeAttr(img.name)}')"
  ...
  // 渲染完成后，如果过滤已开启则异步检测隐藏模糊图
  if (filterBlurry) applyBlurFilter();
}

function toggleTaskCard(headerEl) {
  const arrow = headerEl.querySelector('.task-card-arrow');
  const body = headerEl.nextElementSibling;
  if (body) body.classList.toggle('expanded');
  if (arrow) arrow.classList.toggle('expanded');
}

function taskGoPage(delta) {
  const totalPages = Math.ceil(taskTotal / taskPageSize);
  const newPage = taskPage + delta;
...
async function loadUncategorized() {
  const section = byId('uncategorized-section');
  if (!section) return;

  try {
    const res = await fetch(`/api/v1/admin/cache/uncategorized?page=1&page_size=200`, {
      headers: buildAuthHeaders(apiKey)
    });
    const data = await res.json();
    if (data.status !== 'success') return;

    _lastUncategorizedData = data.items || [];
    renderUncategorized();
  } catch (e) {
    // 忽略
  }
...
function renderUncategorized() {
  ...
  const items = _lastUncategorizedData;

  const countEl = byId('uncategorized-count');
  if (countEl) countEl.textContent = items.length;

  if (items.length > 0) {
    section.classList.remove('hidden');
    const grid = byId('uncategorized-grid');
    if (grid) {
      grid.className = 'uncategorized-grid';
      grid.innerHTML = items.map((item, i) =>
        `<img src="${item.view_url}" alt="" class="uncategorized-thumb" loading="lazy"
          onclick="openLightboxFromUncategorized('${escapeAttr(item.name)}')"
          onerror="this.style.display='none'"
        >`
      ).join('');
    }
...
function toggleUncategorized() {
  const grid = byId('uncategorized-grid');
  const arrow = byId('uncategorized-arrow');
  if (grid) grid.classList.toggle('hidden');
  if (arrow) arrow.style.transform = grid && !grid.classList.contains('hidden')
    ? 'rotate(180deg)' : '';
}
...
// ========== 任务分组视图的 lightbox 支持 ==========
let _lightboxTaskMode = null;
let _lightboxTaskImages = [];
let _lightboxTaskIndex = 0;
let _lightboxTaskId = null;

function _getVisibleTaskImages(taskId) {
  const task = _lastTasksData.find(t => t.task_id === taskId);
  if (!task) return [];
  if (!filterBlurry) return task.images;
  // 过滤模式下，排除已确认模糊的图片
  return task.images.filter(img => !blurCache.get(img.view_url));
}

function _getVisibleUncategorized() {
  if (!filterBlurry) return _lastUncategorizedData;
  return _lastUncategorizedData.filter(i => !blurCache.get(i.view_url));
}

// 从任务分组打开 lightbox（支持翻页和删除）
function openLightboxFromTask(taskId, imgName) {
  const dialog = byId('lightbox-dialog');
  if (!dialog) return;
  _lightboxTaskMode = 'task';
  _lightboxTaskId = taskId;
  _lightboxTaskImages = _getVisibleTaskImages(taskId);
  // 通过图片名称定位在可见列表中的索引
  _lightboxTaskIndex = Math.max(0, _lightboxTaskImages.findIndex(img => img.name === imgName));
  _showTaskLightbox();
  dialog.showModal();
  document.addEventListener('keydown', _taskLightboxKeyHandler);
}

// 从未分类打开 lightbox
function openLightboxFromUncategorized(imgName) {
  const dialog = byId('lightbox-dialog');
  if (!dialog) return;
  _lightboxTaskMode = 'uncategorized';
  _lightboxTaskId = null;
  _lightboxTaskImages = _getVisibleUncategorized();
  _lightboxTaskIndex = Math.max(0, _lightboxTaskImages.findIndex(img => img.name === imgName));
  _showTaskLightbox();
  dialog.showModal();
  document.addEventListener('keydown', _taskLightboxKeyHandler);
...
function _showTaskLightbox() {
  const item = _lightboxTaskImages[_lightboxTaskIndex];
  if (!item) return;
  const img = byId('lightbox-img');
  const counter = byId('lightbox-counter');
  const nameEl = byId('lightbox-name');
  ...
  if (counter) counter.textContent = `${_lightboxTaskIndex + 1} / ${_lightboxTaskImages.length}`;
  if (nameEl) nameEl.textContent = item.name || '';
  if (prevBtn) prevBtn.disabled = _lightboxTaskIndex <= 0;
  if (nextBtn) nextBtn.disabled = _lightboxTaskIndex >= _lightboxTaskImages.length - 1;
}

function _closeTaskLightbox() {
  const dialog = byId('lightbox-dialog');
  if (dialog) dialog.close();
  _lightboxTaskMode = null;
  document.removeEventListener('keydown', _taskLightboxKeyHandler);
}

function _taskLightboxKeyHandler(e) {
  if (!_lightboxTaskMode) return;
  if (e.key === 'ArrowLeft') { e.preventDefault(); _taskLightboxPrev(); }
  else if (e.key === 'ArrowRight') { e.preventDefault(); _taskLightboxNext(); }
  else if (e.key === 'Escape') { e.preventDefault(); _closeTaskLightbox(); }
}

function _taskLightboxPrev() {
  if (_lightboxTaskIndex > 0) {
    _lightboxTaskIndex--;
    _showTaskLightbox();
  }
}

function _taskLightboxNext() {
  if (_lightboxTaskIndex < _lightboxTaskImages.length - 1) {
    _lightboxTaskIndex++;
    _showTaskLightbox();
  }
}

// 覆盖 lightbox 按钮事件，根据模式分发
const _origLightboxDeleteCurrent = lightboxDeleteCurrent;
lightboxDeleteCurrent = async function () {
  if (_lightboxTaskMode) {
    await _taskLightboxDeleteCurrent();
  } else {
    await _origLightboxDeleteCurrent();
  }
};

const _origLightboxDownloadCurrent = lightboxDownloadCurrent;
lightboxDownloadCurrent = function () {
  if (_lightboxTaskMode) {
    const item = _lightboxTaskImages[_lightboxTaskIndex];
    if (!item) return;
    const a = document.createElement('a');
    ...
    _origLightboxDownloadCurrent();
  }
};

const _origLightboxPrev = lightboxPrev;
lightboxPrev = function () { _lightboxTaskMode ? _taskLightboxPrev() : _origLightboxPrev(); };

const _origLightboxNext = lightboxNext;
lightboxNext = function () { _lightboxTaskMode ? _taskLightboxNext() : _origLightboxNext(); };

const _origCloseLightbox = closeLightbox;
closeLightbox = function () { _lightboxTaskMode ? _closeTaskLightbox() : _origCloseLightbox(); };

async function _taskLightboxDeleteCurrent() {
  const item = _lightboxTaskImages[_lightboxTaskIndex];
  if (!item) return;
  const ok = await confirmAction(`确定要删除 ${item.name} 吗？`, { okText: '删除' });
  if (!ok) return;
  const okDel = await requestDeleteLocalFile('image', item.name);
  if (!okDel) { showToast('删除失败', 'error'); return; }
  showToast('删除成功', 'success');

  // 从数据源中移除
  if (_lightboxTaskMode === 'task' && _lightboxTaskId) {
    const task = _lastTasksData.find(t => t.task_id === _lightboxTaskId);
    if (task) {
      task.images = task.images.filter(img => img.name !== item.name);
      task.image_count = task.images.length;
      // 更新分组卡片 DOM
      const card = document.querySelector(`.task-card[data-task-id="${_lightboxTaskId}"]`);
      if (card) {
        if (task.images.length === 0) {
          card.remove();
        } else {
          const countSpan = card.querySelector('.task-img-count');
          const visibleImages = _getVisibleTaskImages(_lightboxTaskId);
          if (countSpan) countSpan.textContent = `${visibleImages.length} 张`;
          const thumb = card.querySelector(`img[data-img-name="${CSS.escape(item.name)}"]`);
          if (thumb) thumb.remove();
        }
      }
    }
  } else if (_lightboxTaskMode === 'uncategorized') {
    _lastUncategorizedData = _lastUncategorizedData.filter(i => i.name !== item.name);
    renderUncategorized();
  }

  // 更新 lightbox 状态
  _lightboxTaskImages = _lightboxTaskImages.filter(img => img.name !== item.name);
  if (_lightboxTaskImages.length === 0) { _closeTaskLightbox(); return; }
  if (_lightboxTaskIndex >= _lightboxTaskImages.length) _lightboxTaskIndex = _lightboxTaskImages.length - 1;
  _showTaskLightbox();
  loadStats();
}

// 删除整个分组
async function deleteTaskGroup(taskId, promptShort) {
  const ok = await confirmAction(`确定要删除分组「${promptShort}」及其所有图片吗？`, { okText: '删除' });
  if (!ok) return;
  try {
    const res = await fetch('/api/v1/admin/cache/task/delete', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json', ...buildAuthHeaders(apiKey) },
      body: JSON.stringify({ task_id: taskId })
    });
    const data = await res.json();
    if (!res.ok || data.status !== 'success') throw new Error('删除失败');
    showToast(`已删除分组（${data.deleted || 0} 张图片）`, 'success');
    // 移除 DOM
    const card = document.querySelector(`.task-card[data-task-id="${taskId}"]`);
    if (card) card.remove();
    _lastTasksData = _lastTasksData.filter(t => t.task_id !== taskId);
    // 更新 tab 计数
    taskTotal = Math.max(0, taskTotal - 1);
    const countEl = byId('task-view-count');
    if (countEl) countEl.textContent = taskTotal;
    loadStats();
  } catch (e) {
...
function toggleBlurFilter() {
  filterBlurry = !filterBlurry;
  const btn = byId('blur-filter-toggle');
  if (btn) btn.classList.toggle('active', filterBlurry);
  if (filterBlurry) {
    applyBlurFilter();
  } else {
    removeBlurFilter();
...
// 合并重复分组
async function mergeTaskGroups() {
  const btn = byId('merge-tasks-btn');
  if (btn) btn.disabled = true;
  try {
    const res = await fetch('/api/v1/admin/cache/tasks/merge', {
      method: 'POST',
      headers: buildAuthHeaders(apiKey),
    });
    const data = await res.json();
    if (!res.ok || data.status !== 'success') throw new Error('合并失败');
    if (data.removed > 0) {
      showToast(`已合并 ${data.merged} 个分组，移除 ${data.removed} 个冗余分组`, 'success');
      loadTasks();
    } else {
      showToast('没有需要合并的重复分组', 'info');
    }
  } catch (e) {
    showToast(e.message || '合并分组失败', 'error');
  } finally {
    if (btn) btn.disabled = false;
  }
}

// 向后兼容：简单的 URL 打开 lightbox（降级模式）
function openLightboxFromUrl(url, name) {
  const dialog = byId('lightbox-dialog');
  if (!dialog) {
  ...
  if (img) img.src = url;
  const nameEl = dialog.querySelector('.lightbox-name');
  if (nameEl) nameEl.textContent = name || '';
  dialog.showModal();
}

// 修改 showCacheSection，在显示图片列表时同时显示视图切换 tab
const _originalShowCacheSection = showCacheSection;
showCacheSection = async function (type) {
  const switcher = byId('view-switcher');
  if (type === 'image') {
    if (switcher) switcher.classList.remove('hidden');
    if (currentCacheView === 'tasks') {
      // 如果当前是任务视图，保持在任务视图
      await _originalShowCacheSection(type);
      const listView = byId('local-cache-lists');
      if (listView) listView.classList.add('hidden');
      const taskView = byId('task-group-view');
      if (taskView) taskView.classList.remove('hidden');
